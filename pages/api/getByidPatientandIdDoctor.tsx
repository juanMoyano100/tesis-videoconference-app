...
const getByidPatientAndidDoctor = async (req: NextApiRequest, res: NextApiResponse) => {
    const { idDoctor, idPatient} = req.query
    
    try {
        await connectMongoDB();
        const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000);
        const request = await Request.findOne({ idDoctor, idPatient, createdAt: { $gte: oneHourAgo } }).sort({ createdAt: -1 }).limit(1);        if (!request) {
